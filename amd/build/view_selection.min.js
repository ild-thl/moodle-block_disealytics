...
/**
   ...
   */
let viewlist=[],courseid=null,versioninfo=null,cardselectionmode="preset";_exports.setViewlist=views=>{viewlist=views};_exports.setVersionInfo=version=>{versioninfo=version};_exports.setCardSelectionMode=mode=>{cardselectionmode=mode};_exports.getVersionInfo=()=>versioninfo;const getViewlist=()=>viewlist.length>0?viewlist:null;_exports.getViewlist=getViewlist;_exports.allViewsEnabled=()=>{const viewlist=getViewlist();if(!viewlist)return!1;if("preset"===cardselectionmode){return["learning-goals-view","assignment-view","planner-view"].every((presetView=>viewlist.some((view=>view.viewname===presetView&&1===view.enabled))))}return viewlist.every((_ref=>{let{enabled:enabled}=_ref;return 0!==enabled}))};_exports.anyViewsEnabled=()=>getViewlist().some((_ref2=>{let{enabled:enabled}=_ref2;return 1===enabled}));_exports.updateViewlist=(modifiedView,write)=>{let updatedViews=[...getViewlist()],index=updatedViews.findIndex((view=>view.viewname===modifiedView));if(-1!==index){if(updatedViews.splice(index,1),"add"===write){const newView={viewname:modifiedView,enabled:1},priorityViews=["learning-goals-view","assignment-view","planner-view"];if(priorityViews.includes(modifiedView)){const insertIndex=((views,priorityViews)=>{let insertIndex=views.findIndex((view=>1===view.enabled&&!priorityViews.includes(view.viewname)));return-1===insertIndex&&(insertIndex=views.findIndex((view=>0===view.enabled))),-1===insertIndex?views.length:insertIndex})(updatedViews,priorityViews);updatedViews.splice(insertIndex,0,newView)}else updatedViews.push(newView)}if("delete"===write){const newView={viewname:modifiedView,enabled:0};updatedViews.push(newView)}}return updatedViews};_exports.setCourseId=id=>{courseid=id};_exports.getCourseId=()=>courseid;let oldViews=[];_exports.oldViews=oldViews;let scrollToElement="";_exports.scrollToElement=scrollToElement;let offsetTopForScroll=0;_exports.offsetTopForScroll=offsetTopForScroll;let scrollTo=!1;_exports.scrollTo=scrollTo;_exports.viewIsOld=view=>oldViews.includes(view);_exports.setOld=view=>{oldViews.includes(view)||oldViews.push(view)};_exports.getOffsetTopForScroll=()=>offsetTopForScroll;_exports.setOffsetTopForScroll=offset=>{_exports.offsetTopForScroll=offsetTopForScroll=offset};_exports.setScrollToElement=idOfView=>{_exports.scrollToElement=scrollToElement=idOfView};_exports.getScrollTo=()=>scrollTo;const setScrollTo=bool=>{_exports.scrollTo=scrollTo=bool};_exports.setScrollTo=setScrollTo;_exports.getScrollToElement=()=>scrollToElement;_exports.unsetOld=view=>{if(oldViews.includes(view)){let index=oldViews.indexOf(view);oldViews.splice(index,1)}};_exports.selectors={views:{selectEveryViewContainer:'[data-containertype="block_disealytics/view-container"]'}};_exports.getViewSelectors=viewname=>({selectViewClass:".block_disealytics-container-"+viewname,selectExpandableClass:".block_disealytics-expandable-"+viewname})}));
